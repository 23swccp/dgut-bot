...
`yxy_capture_fixed.py` 仅用于本机调查；输入 `s` 开始抓包后，默认每 120 秒（2 分钟）刷新一次东莞理工/优学院页面，以减少长时间监听时的日志噪声，设 `AUTO_REFRESH_INTERVAL=0` 可关闭自动刷新。脚本生成的原始抓包可能含 Cookie、Token、用户与课程信息，不属于该实验的提交内容，也不得公开分享。
