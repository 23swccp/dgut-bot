...
Python 直连上游与正常 Chromium 请求可能在 HTTP 版本、TLS 指纹、浏览器专用请求头、Cookie 上下文和连接复用方面不同；结构化答题提示及后续测验提交也可能被上游或学校日志记录。因此不得将该实验描述为不可检测，也不实现伪造浏览器/TLS 指纹、随机鼠标轨迹、随机停留或其他用于模拟真人和规避平台监测的逻辑。
