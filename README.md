...
启动后先在终端检测浏览器；未找到时会弹出文件选择窗口，请选择浏览器的 `.exe` 文件（如 `msedge.exe` 或 `chrome.exe`）。取消选择后也可在终端粘贴程序路径或安装文件夹，按回车重新选择，输入 q 退出。确认有效后才打开网页。后续登录等操作务必在程序打开的浏览器进行，**不要新开另一个进程的浏览器**。

## 课程签到

签到模块会读取课程、监测当天的课堂活动，并在发现支持的签到类型后自动尝试签到。二维码活动没有下发码值时，程序会尝试以空码提交，并显示学校接口返回的结果；空码能否成功取决于学校接口，尚无手机抓包验证。目前签到方面仍在开发。

### 1. 启动并完成登录

- 启动程序并进入课程签到模块。
- 程序会自动读取本机登录缓存，通过调试浏览器中对应的学校页面查询课程与活动；提交签到时使用已验证的登录信息直接请求学校接口。
...
- 如果出现登录页，请在程序打开的独立浏览器中完成登录；课程读取成功后，程序会在签到事件流和模块侧边栏显示登录问候。程序尝试从 `USERINFO` Cookie 读取姓名；若该账号的 Cookie 未提供可识别的姓名字段，则依次显示账号或用户 ID。旧登录缓存可能只有 Token 和用户 ID，重新登录后才有机会读取姓名；当前尚未用真实账号确认姓名字段一定存在。
- 浏览器中的 Cookie 只作为候选登录信息；程序成功读取课程后才会更新本地缓存。若候选 Cookie 已失效并返回登录重定向或 HTTP 401，程序会丢弃失效缓存，并等待浏览器产生新的登录信息。

课程列表经 `lms.dgut.edu.cn` 页面查询，课堂与活动经 `application.dgut.edu.cn` 页面查询；签到提交直连 `application.dgut.edu.cn`。程序会为浏览器查询选择与接口同源的标签页。

![](./docs/login-flow.svg)

> 登录提示：请始终在程序打开的浏览器中登录。日常使用的其它浏览器与程序的登录状态并不共用。签到监测期间请勿关闭程序打开的 LMS/Application 学校页面；这些页面可以留在后台。
...
签到成功、重复签到、跳过原因、登录失效和错误等关键变化会作为独立节点加入事件流。如果缺少对应学校页面、浏览器被关闭或登录状态失效，本轮请求会明确失败。
...
选人活动与签到不同：学生端可以查询选人结果，但现有程序不会参与抽选，也不能阻止服务器选中某人。


## 课件学习

刷课模块用于辅助处理课件中的视频、文档、章节切换和提示弹窗。进入模块后，程序会在登录有效时自动扫描当前课程中的未完成课件；扫描结果在本次运行内缓存约 5 分钟。
...
自动答题默认关闭，需要在设置 → 刷课中为本次运行明确启用。开启总开关后，可以分别启用选择题、判断题和填空题。成功启动一次刷课任务后，总开关会立即恢复为关闭；当前任务仍按启动时的选择运行，下次启动则需要重新开启。
...
> AI 请求或答案格式连续两次失败，以及遇到尚不支持的题型时，程序会在触碰页面前退回旧规则：选择题选择 `C`，判断题选择“错误”，填空题填写 `,`。页面填写开始后若出现错误，程序会停止在当前页。

> 课程 AI 请求、测验提交和签到操作都可能由学校或平台记录。刷课只对已经校验的页面操作加入有限的鼠标轨迹和短停顿；课程查询经 Chromium 页面请求，签到提交由 Python 直连学校接口，但这些措施都不能保证自动化行为不可识别。

## 作业

进入程序侧边栏的“作业”后，程序会扫描当前账号的全部课程，并列出平台标记为“未互评”或“互评中”的作业。“未互评”表示仍需处理。课程列表默认收起，点击课程名右侧的下拉三角显示该课程的作业，再点击每条作业右侧的下拉三角展开匿名作业的评分和评语。扫描允许单门课程失败，结果在本次运行中缓存约五分钟，也可以点击“扫描全部课程”强制刷新。

程序内评分会为尚未填写的互评预填 100 分（若作业满分低于 100，则预填该满分），保留已有评分并允许在平台开放修改时调整。评分项存在时必须逐项填写且合计等于总分；评语可选。点击“一键提交”会提交当前作业下全部可编辑互评，程序会在真正写入优学院前再次确认；若多份提交中途失败，会明确显示已成功的份数，已经成功的部分不会自动撤回。

扫描完成后，程序会先逐个只读检查详情接口；只有平台明确返回当前处于互评时间、仍有未评分内容且作业尚未到期时，才会显示浏览器原生确认框，并逐项列出将填写 100 分的课程名和作业名。只有点击“确定”后才会写入平台；点击“取消”后本次扫描不再重复询问。提交前会再次读取服务端状态，只处理尚未评分且平台仍允许互评的内容，不覆盖已有评分。尚未开始互评、截止时间缺失或已过期、作业满分不足 100、评分项无法合计为 100 的作业不会提交，失败结果会按作业显示。

如需输入作业内容，请先在程序启动的浏览器中打开优学院“写作业”页面。状态显示“已识别输入框”后，在程序输入框中填写内容并点击“发送”（也可按 `Ctrl + Enter`）。程序会把文字追加到作业编辑框末尾，支持换行；请在作业页面核对后自行提交。

作业内容输入支持页面中的 TinyMCE 作业编辑框，单次及作业框总内容最多 5000 字。若同时打开多个可输入的作业页面，程序会暂停发送，避免写入错误作业。输入内容仅用于本次发送，不保存到程序配置或日志。互评界面不显示被评学生身份，附件只展示文件名；提交时的学生、互评和评分项标识均由程序使用提交前重新读取的服务端数据绑定，不接受前端直接指定。
...
启用“开机时打开校园网登录页”并保存设置后，程序会在当前用户的注册表 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 中创建 `DgutBotCampusLogin` 启动项；关闭开关并保存后会移除该项。开机后，它会立即使用默认浏览器打开 `https://login.dgut.edu.cn/eportal/index.jsp`。该功能不等待网络、不判断当前是否已联网；
...
小皮卡通过 Velopack 从 GitHub 检查更新。若更新失败或速度较慢，请开启支持 GitHub 的网络加速器(如steam++,UU加速器,FastGitHub或VPN)；首次安装器也可从 [Gitee 下载页](https://gitee.com/swccq23/dgut-bot/releases) 获取。
...
本项目采用 [MIT License]({{REPO_URL}}/blob/main/LICENSE) 开源。如需反馈问题或提建议，请通过 GitHub Issues 提交,或私信贴吧用户:785434816
...
发布、自动更新和 Gitee 镜像规则见 `发布与更新维护.md`；当前代码维护地图见 `交接文档.md`；2026-09-22 手机抓包结论见 `签到与选人抓包记录.md`。
