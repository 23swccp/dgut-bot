...
在程序启动的浏览器中打开优学院“写作业”页面，再进入程序侧边栏的“作业输入”。状态显示“已识别输入框”后，在程序输入框中填写内容并点击“发送”（也可按 `Ctrl + Enter`）。程序会把文字追加到作业编辑框末尾，支持换行；请在作业页面核对后自行提交。

当前支持页面中的 TinyMCE 作业编辑框，单次及作业框总内容最多 5000 字。若同时打开多个可输入的作业页面，程序会暂停发送，避免写入错误作业。输入内容仅用于本次发送，不保存到程序配置或日志。
